...
class MSG_GAN:
    ...
    def create_grid(self, samples, img_files):
        ...
        # dynamically adjust the colour of the images 压到0-1
    ...
    def train(self, data, gen_optim, dis_optim, loss_fn, normalize_latents=True,
        ...
        fixed_input = torch.randn(num_samples, self.latent_size).to(self.device)  # 36*512
        if normalize_latents:
            fixed_input = (fixed_input / fixed_input.norm(dim=-1, keepdim=True)
                           * (self.latent_size ** 0.5))  # 512
        ...
        for epoch in range(start, num_epochs + 1):
            ...
            for (i, batch) in enumerate(data, start=1):
                ...
                if i % (int(limit / feedback_factor) + 1) == 0 or i == 1:
                    # Avoid div by 0 error on small training sets
